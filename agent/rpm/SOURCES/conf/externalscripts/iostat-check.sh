...
# source data file
SOURCE_DATA=/var/log/zabbix/iostat-data
...
case $ZBX_REQ_DATA in
  ...
  tps)        
              r_s=$(grep -E "^$ZBX_REQ_DATA_DEV " $SOURCE_DATA | tail -1 | awk '{print $4}')
              w_s=$(grep -E "^$ZBX_REQ_DATA_DEV " $SOURCE_DATA | tail -1 | awk '{print $5}')
              echo $(echo "$r_s + $w_s"|bc)
              ;;
